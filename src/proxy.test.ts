...
describe("proxy matcher", () => {
  it("does not run authentication for the Meta webhook", () => {
    expect(unstable_doesMiddlewareMatch({ config, nextConfig: {}, url: "/api/webhooks/whatsapp" })).toBe(false);
  });
  ...
  it("continues to protect private application routes", () => {
    ...
    expect(unstable_doesMiddlewareMatch({ config, nextConfig: {}, url: "/api/customers" })).toBe(true);
  });
});
